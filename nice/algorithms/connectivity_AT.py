...
def epochs_compute_wsmi(epochs, kernel, tau, tmin=None, tmax=None,
    ...
    if backend == 'python':
        ...
        wsmi, smi, pxy_track = _wsmi_python(sym, count, wts)
        wsmi_AT, smi_AT, pxy_track_AT = _wsmi_across_trials(sym, count, wts)
    elif backend == 'openmp':
    ...
    return wsmi, smi, sym, count, pxy_track, wsmi_AT, smi_AT, pxy_track_AT
...
def _wsmi_python(data, count, wts):
    ...
    return wsmi, smi, pxy_track
      
def _wsmi_across_trials(data, count, wts):
    """Compute wsmi"""
    nchannels, nsamples, ntrials = data.shape
    nsymbols = count.shape[1]
    smi = np.zeros((nchannels, nchannels), dtype=np.double)
    wsmi = np.zeros((nchannels, nchannels), dtype=np.double)
    pxy_track = []
    aux1 = []
    sc1track = []
    sc2track = []
    agg_count = np.sum(count, axis=-1) / ntrials
    for channel1 in range(nchannels):
        for channel2 in range(channel1 + 1, nchannels):
            pxy = np.zeros((nsymbols, nsymbols))
            for trial in range(ntrials):
            ...
            for sc1 in range(nsymbols):
                for sc2 in range(nsymbols):
                    if pxy[sc1, sc2] > 0:
                        aux = pxy[sc1, sc2] * np.log(
                            pxy[sc1, sc2] /  # noqa
                            agg_count[channel1, sc1] /  # noqa
                            agg_count[channel2, sc2])
                        sc1track.append(sc1)
                        sc2track.append(sc2)
                        aux1.append(aux)
                        smi[channel1, channel2] += aux
                        wsmi[channel1, channel2] += \
                            (wts[sc1, sc2] * aux)
